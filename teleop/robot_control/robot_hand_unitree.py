...
class Dex3_1_Controller_XR:
    def __init__(self, left_gripper_value_in, right_gripper_value_in, 
                        left_thumb_value_in, right_thumb_value_in,
                        left_aux_array_in, right_aux_array_in,     # controller buttons
                        dual_hand_data_lock = None, dual_hand_state_array_out = None,
                        dual_hand_action_array_out = None, fps = 100.0, Unit_Test = False, simulation_mode = False):    
        """
        По нажатию на джойстик сжимаем руку! Вместо распознавания с камеры положения рук.
        ...
        simulation_mode: Whether to use simulation mode (default is False, which means using real robot)
        """
        logger_mp.info("Initialize Dex3_1_Controller_XR...")

        self.fps = fps
        self.Unit_Test = Unit_Test
        self.simulation_mode = simulation_mode
        # comment it to fix ValueError: URDF dir ../../assets not exists.
        # if not self.Unit_Test:
        #     self.hand_retargeting = HandRetargeting(HandType.UNITREE_DEX3)
        # else:
        #     self.hand_retargeting = HandRetargeting(HandType.UNITREE_DEX3_Unit_Test)
        ...
        # fix syntax (gripper --> hand for dex3)
        ...
        logger_mp.info("Initialize Dex3_1_Controller_XR OK!\n")
    ...
    def control_process(self, left_gripper_value_in, right_gripper_value_in, 
                              left_thumb_value_in, right_thumb_value_in,
                              left_aux_array_in, right_aux_array_in,      # controller buttons
            ...
            # PID values (for msg init)
            # q = 0.0; dq = 0.0; tau = 0.0; kp = 1.5; kd = 0.2
            q = 0.0; dq = 0.0; tau = 0.0; kp = 0.5; kd = 0.1
            ...
            for id in Dex3_1_Left_JointIndex:
                ...
                self.left_msg.motor_cmd[id].q = q; self.left_msg.motor_cmd[id].kp = kp; self.left_msg.motor_cmd[id].kd = kd

            self.right_msg = unitree_hg_msg_dds__HandCmd_()
            for id in Dex3_1_Right_JointIndex:
                ris_mode = self._RIS_Mode(id = id, status = 0x01)
                self.right_msg.motor_cmd[id].mode = ris_mode._mode_to_uint8()  
                self.right_msg.motor_cmd[id].q = q; self.right_msg.motor_cmd[id].kp = kp; self.right_msg.motor_cmd[id].kd = kd 
            
            SMOOTH_FACTOR = 0.1 
            last_debug_time = time.time() # timer for debug prints
            ...
            try:
                while self.running:
                    ...
                    with left_gripper_value_in.get_lock():
                        l_fingers_raw = left_gripper_value_in.value
                    with right_gripper_value_in.get_lock():
                        r_fingers_raw = right_gripper_value_in.value
                    with left_thumb_value_in.get_lock():
                        l_thumb_raw = left_thumb_value_in.value
                    with right_thumb_value_in.get_lock():
                        r_thumb_raw = right_thumb_value_in.value

                    # --- (AUX) DATA READING ---
                    with left_aux_array_in.get_lock():
                        l_aux = left_aux_array_in[:]
                    with right_aux_array_in.get_lock():
                        r_aux = right_aux_array_in[:]

                    # Unpacking (StickX, StickY, Click, A, B)
                    lx, ly = l_aux[0], l_aux[1]
                    rx, ry = r_aux[0], r_aux[1]
                    # Buttons
                    l_btns = f"Ck:{int(l_aux[2])} A:{int(l_aux[3])} B:{int(l_aux[4])}"
                    r_btns = f"Ck:{int(r_aux[2])} A:{int(r_aux[3])} B:{int(r_aux[4])}"

                    # State reading (for torque feedback, NOT USED NOW)
                    state_data = np.concatenate((np.array(left_hand_state_array[:]), np.array(right_hand_state_array[:])))

                    # --- DEBUG PRINT (all controllers values) ---
                    if time.time() - last_debug_time > 1:
                        print(f"\n[DEBUG] Inputs:")
                        print(f"  LEFT : Trig:{l_fingers_raw:5.2f} | Grip:{l_thumb_raw:5.2f} | Stk:({lx:5.2f},{ly:5.2f}) | {l_btns}")
                        print(f"  RIGHT: Trig:{r_fingers_raw:5.2f} | Grip:{r_thumb_raw:5.2f} | Stk:({rx:5.2f},{ry:5.2f}) | {r_btns}")
                    ...
                    # target_r_fingers = r_fingers_clamped / 10.0
                    # target_l_thumb   = l_thumb_clamped / 10.0
                    # target_r_thumb   = r_thumb_clamped / 10.0
                    ...
                    r_fingers = r_fingers * (1 - SMOOTH_FACTOR) + target_r_fingers * SMOOTH_FACTOR
                    l_thumb   = l_thumb   * (1 - SMOOTH_FACTOR) + target_l_thumb   * SMOOTH_FACTOR
                    r_thumb   = r_thumb   * (1 - SMOOTH_FACTOR) + target_r_thumb   * SMOOTH_FACTOR

                    # 4. INTERPOLATION
                    # --- LEFT HAND ---
                    left_q_target[IDX_THUMB] = DEX3_OPEN_POSE_LEFT[IDX_THUMB] + \
                        (DEX3_CLOSE_POSE_LEFT[IDX_THUMB] - DEX3_OPEN_POSE_LEFT[IDX_THUMB]) * l_thumb
                    
                    left_q_target[IDX_FINGERS] = DEX3_OPEN_POSE_LEFT[IDX_FINGERS] + \
                        (DEX3_CLOSE_POSE_LEFT[IDX_FINGERS] - DEX3_OPEN_POSE_LEFT[IDX_FINGERS]) * l_fingers

                    # --- RIGHT HAND ---
                    right_q_target[IDX_THUMB] = DEX3_OPEN_POSE_RIGHT[IDX_THUMB] + \
                        (DEX3_CLOSE_POSE_RIGHT[IDX_THUMB] - DEX3_OPEN_POSE_RIGHT[IDX_THUMB]) * r_thumb
                    
                    right_q_target[IDX_FINGERS] = DEX3_OPEN_POSE_RIGHT[IDX_FINGERS] + \
                        (DEX3_CLOSE_POSE_RIGHT[IDX_FINGERS] - DEX3_OPEN_POSE_RIGHT[IDX_FINGERS]) * r_fingers

                    # 5. EXECUTION
                    left_actual_action  = left_q_target
                    right_actual_action = right_q_target

                    # Shared memory update (for logs/debug)
                    action_data = np.concatenate((left_actual_action, right_actual_action))
                    if dual_hand_state_array_out and dual_hand_action_array_out:
                        with dual_hand_data_lock:
                            dual_hand_state_array_out[:] = state_data
                            dual_hand_action_array_out[:] = action_data                
                    ...
                    # Command send
                    self.ctrl_dual_hand(left_actual_action, right_actual_action)
                ...
                import traceback
                logger_mp.error(f"Dex3 Controller CRASHED: {e}")
                traceback.print_exc()
            finally:
                logger_mp.info("Dex3_1_Controller_XR has been closed.")
